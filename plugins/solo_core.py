#!/usr/bin/env python3
"""Pure Arena Run rules for v5.

No minqlx dependency belongs in this file. Only upgrades with a concrete v5
runtime consumer are eligible to roll in the alpha.
"""
...
UPGRADES = [
    ...
    {"id":"haste_1","name":"Fleet Footed","rarity":"uncommon","max":2,"text":"Gain Haste","effects":{"haste":1}},
    ...
    {"id":"rocket_damage","name":"Heavy Warheads","rarity":"uncommon","max":4,"text":"+15% rocket damage","effects":{"rocket_mult":0.15}},
    ...
    {"id":"rail_combo","name":"Perfect Shot","rarity":"rare","max":1,"text":"Every third rail hit gets bonus damage","effects":{"rail_combo":1}},
    {"id":"plasma_damage","name":"Hot Plasma","rarity":"uncommon","max":4,"text":"+15% Plasma Gun damage","effects":{"plasma_mult":0.15}},
    {"id":"ammo_regen","name":"Bottomless","rarity":"rare","max":1,"text":"Ammo refills on spawn and after a clear","effects":{"bottomless":1}},
...
def round_plan(state: RunState) -> dict:
    ...
    if boss:
        ...
        return {"boss":True,"theme":"boss","count":1,"skill":min(5,3+diff+tier//2),"bots":[names[(tier-1)%len(names)]],"health":450+tier*225,"armor":150+tier*100,"damage_mult":1.0+tier*0.12}
    elite = r >= 7 and r % 3 == 0
    if r % 7 == 0: theme = "rail"
    elif r % 6 == 0: theme = "rocket"
    elif r % 4 == 0: theme = "lg"
    elif elite: theme = "elite"
    else: theme = "normal"
    count = min(8, 2 + (r - 1) // 2 + max(0, diff))
    skill = max(1, min(5, 2 + (r - 1) // 4 + diff))
    bots = [rng.choice(BOT_ROSTER[:-1]) for _ in range(count)]
    return {"boss":False,"theme":theme,"count":count,"skill":skill,"bots":bots,"health":100+max(0,r-3)*8,"armor":max(0,r-4)*5,"damage_mult":1.0+max(0,r-5)*0.025}
